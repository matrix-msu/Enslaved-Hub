 ...
 *          type (options=[LatLon | UTM | Address]): string of location type to search.
 *          if type = LatLon
 *              lat: number of latitude to search
 *              lon: number of longitude to search
 *          if type = UTM
 *              zone: string of UTM zone to search
 *              east: number of easting to search
 *              north: number of northing to search
 *          if type = Address
 *              address: string of text to search
 *              range: number of radius from location center to search
 *          example: array("type"=>"LatLon", "lat"=>"latitueValue", "lon"=>"longitudeValue")
 *      Associator
...
function koraWrapperSearch(
  ...
  $sizeDateAndDataOptions = []  //array to specify returns of no data and sizes. Also used for date range searches. Syntax above.
    ...
    foreach( $formIds as $formId ){
        ...
        $formQuery['token'] = TOKEN; //$GLOBALS['FORMS_CONFIG'][$formId]['token'];
