...
<?php
    ...
    $prefix = '<p>';
    if (substr($kid, 0, strlen($prefix)) == $prefix) {
        $kid = substr($kid, strlen($prefix));
    }
    $substring = '</p>';
    if (substr($kid,-strlen($substring))===$substring){
        $kid = substr($kid, 0, strlen($kid)-strlen($substring));
    }
    $_GET['kid'] = $kid;
    // $koraResult = ALL_STORIES[$kid];
 ...
 if (isset($_GET['kid']) && preg_match("/^[0-9A-F]+-[0-9A-F]+-[0-9A-F]+(-[0-9A-F]+)*$/", $_GET['kid'])) {
     ...
     $story = ALL_STORIES[ $_GET['kid'] ];


     $images = [];
     $caption = [];
     if (isset($story['Images']) && is_array($story['Images']) ) {
         if(isset($story['Images']['localName'])){
             $images[] = $story['Images']['localName'];
             $caption[] = $story['Images']['originalName'];
         }else{
   ...
   $url = BASE_URL."fullStory/".$_GET['kid']."/";
 ?>
 <!-- Full Story page-->
 ...
 <div class="container header fullstory">
   <div class="image-container fullStory-page image-only">
     <img class="header-background fullStory-page" src="<?php echo BASE_URL;?>assets/images/<?php echo $GLOBALS['bg'][$GLOBALS['randIndex']];?>" alt="Enslaved Background Image">
