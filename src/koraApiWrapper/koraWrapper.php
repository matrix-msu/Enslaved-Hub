<?php
// include_once(BASE_URL."/function/functions.php");
/**
...
function koraWrapperSearch(
  ...
  $sizeDateAndDataOptions = []  //array to specify returns of no data and sizes. Also used for date range searches. Syntax above.
    ...
    foreach( $formIds as $formId ){
		...
		$formQuery['form'] = $formId;
		$formQuery['bearer_token'] = TOKEN;
	    //$formQuery['return_fields'] = $returnedFields[$formIndex];
		...
		if ( !empty($fieldsToQuery[$formIndex]) ){
			if ( $query[$formIndex] !== "" && is_string($query[$formIndex]) ){
				if( $fieldsToQuery[$formIndex]=='kid'||$fieldsToQuery[$formIndex]=='KID'){ //kid search
                    ...
                    //$formattedQuery['key_words'] = array('Female','Ogalay');
                    // $formattedQuery['key_words'] = array('Female');
                    $formattedQuery['key_method'] = 'AND';
				...
				for( $fieldIndex=0; $fieldIndex<count($fieldNames); $fieldIndex++){
					...
					$queryField = key($fieldNames[$fieldIndex]);
					$queryType = $fieldNames[$fieldIndex][$queryField];
					if( !in_array($queryType,array('TEXT','RICH TEXT','List','DOCUMENTS','GALLERY','PLAYLIST','VIDEO','3-D MODEL','Multi-Select List')) ){
						$singleFormFieldsArray[$queryField] = array('input'=>$query[$formIndex][$fieldIndex]);
					}else if($queryType == 'Multi-Select List'){
						$singleFormFieldsArray[$queryField] = array('input'=>array($query[$formIndex][$fieldIndex]));
					}else{
						$singleFormFieldsArray[$queryField] = array('input'=>$query[$formIndex][$fieldIndex]);
					}
				}
				$singleQuery['adv_fields'] = $singleFormFieldsArray;
				$formattedQuery[] = $singleQuery;
	        }
			$formQuery['queries'] = $formattedQuery;
    ...
    $ch = curl_init(KORA_SEARCH_URL); # initialize the curl
    curl_setopt($ch, CURLOPT_POST, 1); # Allows the use of CURLOPT_POSTFIELDS
    curl_setopt($ch, CURLOPT_POSTFIELDS, $data); # Alows $data to be sent to the server when the curl is executed
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); # this means curl_exec will return the value on success
    $result = curl_exec($ch);
    curl_close($ch);
    //print_r($result);die;
    return $result;
...
**/
	...
	if ( strtoupper($type) == 'CREATE' ){//Create a record
      ...
      $data = ['form' => $form,
      	'bearer_token' => TOKEN, 'fields' => $query];
	...
	if ( strtoupper($type) == 'UPDATE' ){//Update a record
      ...
      $data = ['_method' => 'put', 'form' => $form,
      	'bearer_token' => TOKEN, 'kid' => $kids,'fields' => $query];
	...
	if ( strtoupper($type) == 'UPLOAD' ){//Upload a file
		...
		$data = ['_method' => 'put', 'form' => $form,
			'bearer_token' => TOKEN, 'kid' => $kids,'fields' => json_encode($fieldArray),'zip_file' => $file];
		$ch = curl_init();
